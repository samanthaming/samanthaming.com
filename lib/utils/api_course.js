...
export async function fetchCoursePrevNext({
  ...
  // fetch first content
  if (!next) {
    next = store.state.course[stateName]; // TODO: update to getter structure, so consistent with tidbit/blog
...
export async function dispatchRecentLesson({
  ...
  contentPath,
  dispatchType,
  fetchRecentLesson, // TODO: change to fetchRecentLessonFunction
}) {
