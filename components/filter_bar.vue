<template>
  <div
    class="px-2 py-4 lg:pt-5 lg:pb-4 border-b shadow-lg flex justify-center font-head space-x-6 md:space-x-7 lg:space-x-5"
    :class="[colorOption.background, colorOption.border]"
  >
    <button
      v-for="({ value, text }, index) in filterOptions"
      :key="index"
      class="inline-flex items-center justify-center w-24 sm:w-28 md:w-32 py-1.5 border-2 text-xs sm:text-sm md:text-base font-semibold rounded-full shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 hover:text-ink"
...
const COLOR_OPTION = {
  orange: {
    ...
    background: 'bg-orange-lighter bg-opacity-40',
    ...
    active: 'bg-orange border-orange text-orange-black',
  },
  green: {
    border: 'border-green-light',
    background: 'bg-green-lightest bg-opacity-50',
